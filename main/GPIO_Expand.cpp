...
#define expanderAddr 0x20

void expander_init() {
    // Set all pins default to output
    I2C_Write8(expanderAddr, 0x00, 0x00);
    I2C_Write8(expanderAddr, 0x01, 0x00);
    //Configure pin behavior
    I2C_Write8(expanderAddr, 0x0A, 0x00); //Check
    I2C_Write8(expanderAddr, 0x0B, 0x00); //Check
    // Set all PUR default to off
    I2C_Write8(expanderAddr, 0x0C, 0x00);
    I2C_Write8(expanderAddr, 0x0D, 0x00);
}
...
void set_dir_bit(uint8_t pin, uint8_t bit){
    ...
    uint8_t data = I2C_Read8(expanderAddr, port);
    ...
    I2C_Write8(expandAddrW, data, port);
}

void set_dir_reg(uint8_t byte, uint8_t port){
    // Writes to the entire dir register
    if(port == this.A)
        I2C_Write8(expanderAddr, byte, 0x00);
    else if(port == this.B)
        I2C_Write8(expanderAddr, byte, 0x01);
}
...
uint8_t get_dir_bit(uint8_t pin){
    ...
    // reads bit
    if(pin == this.A0)
        return (I2C_Read8(expanderAddr, port) & 0x01);
    else if(pin == this.A1)
        return (I2C_Read8(expanderAddr, port) & 0x02) >> 1;
    else if(pin == this.A2)
        return (I2C_Read8(expanderAddr, port) & 0x04) >> 2;
    else if(pin == this.A3)
        return (I2C_Read8(expanderAddr, port) & 0x08) >> 3;
    else if(pin == this.A4)
        return (I2C_Read8(expanderAddr, port) & 0x10) >> 4;
    else if(pin == this.A5)
        return (I2C_Read8(expanderAddr, port) & 0x20) >> 5;
    else if(pin == this.A6)
        return (I2C_Read8(expanderAddr, port) & 0x40) >> 6;
    else if(pin == this.A7)
        return (I2C_Read8(expanderAddr, port) & 0x80) >> 7;
    else
        return -1;
}
...
uint8_t get_dir_reg(uint8_t port){
    ...
    if(port == this.A)
        return I2C_Read8(expanderAddr, 0x00);
    else if(port == this.B)
        return I2C_Read8(expanderAddr, 0x01);
}
...
void set_pur_reg(uint8_t byte, uint8_t port){
    ...
    if(port == this.A)
        I2C_Write8(expanderAddr, byte, 0x0C);
    else if(port == this.B)
        I2C_Write8(expanderAddr, byte, 0x0D);
}
...
uint8_t read_port(uint8_t port){
    // Reads entire port
    if(port == this.A)
        return I2C_Read8(expanderAddr, 0x13);
    else if(port == this.B)
        return I2C_Read8(expanderAddr, 0x14);
}
