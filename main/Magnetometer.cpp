...
void Mag_Init() {
    if(uint8_t i = Mag_Read8(MAG_WHOAMI)!=MAG_WHOAMI_VAL){
        printf("Error for Mag device ID: Expected C7, Got %X\n", i);
        return;
    }

    //standby mode
    Mag_Write8(0x0, MAG_M_CTRL_REG1);
    //auto-cal disabled
    //no one-shot mag reset
    //no one-shot mag measurement
    //8x oversampling
    //hybrid mode with accel and mag active
    Mag_Write8(0x1F, MAG_CTRL_REG1);
    ...
    Mag_Write8(0x0D, MAG_M_CTRL_REG1);
...
void Mag_Loop(void* ptr){
    while (true){
        while((Mag_Read8(MAG_M_DR_STATUS)&!0x8u)!=0){vTaskDelay(1 / portTICK_PERIOD_MS);} //delay until ready
        magReady = 0;
        ...
        magReady = 1;
        vTaskDelay(5 / portTICK_PERIOD_MS); //delay 5ms for 200hz
    }
}
